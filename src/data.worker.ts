...
self.onmessage=async(e:MessageEvent<{type:string;id:number;query:Query}>)=>{
  try{
    if(e.data.type==='init'){
      const mr=await fetch(import.meta.env.BASE_URL+'data/manifest.json',{cache:'no-cache'});if(!mr.ok)throw Error('Could not load archive information.');const manifest:Manifest=await mr.json();
      const response=await fetch(import.meta.env.BASE_URL+'data/'+manifest.index);if(!response.ok)throw Error('Could not load the archive index.');const data:Index=await response.json();if(data.schema!==1)throw Error('Unsupported archive version.');engine=new Engine(data);
