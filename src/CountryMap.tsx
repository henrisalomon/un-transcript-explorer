...
export default function CountryMap({rows,total,onCountry}:{rows:CountryRow[];total:number;onCountry:(s:string)=>void}){
 ...
 const lookup=new Map(rows.map(r=>[r.code,r])),max=Math.max(1,...rows.map(r=>r.meetings));
 ...
 const share=(row:CountryRow)=>`${total?(row.meetings/total*100).toFixed(1):'0.0'}% of filtered meetings`;
 const detail=(row:CountryRow)=>`${fmt(row.meetings)} meeting${row.meetings===1?'':'s'} · ${share(row)}`;
 ...
 return <div className="map-area">
  <div className="map-canvas" onKeyDown={e=>{if(e.key==='Escape')setHover(null)}} onMouseLeave={()=>setHover(null)}>
   {error?<p className="empty">Map unavailable. Use the country list to explore participation.</p>:!paths.length?<p className="map-loading">Loading world map…</p>:<svg viewBox="0 0 1000 506" className="world-map" aria-label="World map: meetings by country">{paths.map(f=>{const row=lookup.get(f.code);return <path key={f.code} d={f.path} fill={row?color(row.meetings):'var(--map-empty)'} role={row?'button':undefined} tabIndex={row?0:undefined} aria-label={`${row?.name||f.name}: ${row?detail(row):'no matching data'}`} onMouseEnter={()=>setHover(f)} onFocus={()=>setHover(f)} onBlur={()=>setHover(null)} onClick={()=>row&&onCountry(f.code)} onKeyDown={e=>{if(row&&(e.key==='Enter'||e.key===' ')){e.preventDefault();onCountry(f.code)}}}/>})}</svg>}
   {hover&&<div className="map-tooltip" role="tooltip"><strong>{hovered?.name||hover.name}</strong>{hovered?<><span>{fmt(hovered.meetings)} meeting{hovered.meetings===1?'':'s'}</span><span>{share(hovered)}</span></>:<span>No matching data</span>}</div>}
  </div>
  <div className="map-scale"><span className="scale-title">Meetings by country</span>{rows.length>0&&<div className="scale-ramp"><div style={{background:`linear-gradient(to right, ${color(1)}, ${color(max)})`}}/><div className="scale-ticks">{ticks.map(n=><span key={n}>{fmt(n)}</span>)}</div></div>}<span className="scale-empty"><i className="no-data"/>No matching data</span></div>
