...
export class Engine{
  ...
  query(q:Query):Result{
    ...
    return {durationSeconds,missingDurations,monthly,meetings:meetingIds.size,interventions,named:person.size,unnamed,countries,categories:[...categories].sort((a,b)=>b[1]-a[1]),persons:people.slice(q.page*25,q.page*25+25),personCount:people.length,profile,elapsed:performance.now()-start};
  }
}
