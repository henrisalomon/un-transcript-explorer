...
class ExportTests(unittest.TestCase):
 def test_normalization_text_and_failed_export_preserves_snapshot(self):
  with tempfile.TemporaryDirectory() as root:
   ...
   obj={'video':{'slug':'sc/1','date':'2026-01-01','title':'Test'},'transcript':{'language':'en','data':[{'paragraphs':[{'sentences':[{'text':'Original words.','topics':[{'key':'t','label':' Topic '},{'key':'t','label':'topic'}]}]}],'speaker':{'name':' A  Person ','affiliation':'FRA','affiliation_full':'France'},'start':12.5}]}}
   (archive/'en/1.json').write_text(json.dumps(obj));(archive/'meetings.jsonl').write_text('{}\n{}\n')
   manifest=mod.export(archive,out);idx=json.loads((out/manifest['index']).read_text());self.assertEqual(manifest['topics'],1);self.assertEqual(idx['speakers'][0]['name'],'A Person');self.assertEqual(json.loads((out/idx['meetings'][0]['file']).read_text()),['Original words.'])
